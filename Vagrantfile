...
NETWORK_SIZE = ENV['NETWORK_SIZE'] || 2
...
def define_srv(config, name, tags)
  config.vm.define name do |srv|
      ...
      config.vm.synced_folder '.', '/vagrant', type: 'rsync',
        rsync__exclude: ['.git/', 'agent', '.rubocop.yml', '.travis.yml', 'dummy.box', 'LICENSE', '*.log', 'ansible']
